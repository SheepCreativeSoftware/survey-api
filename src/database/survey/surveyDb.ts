...
const updateSurveyInDb = async ({
	...
	WHERE creation_token = ?`,
		[choicesType, creatorName, endDate, surveyName, surveyDescription, creationToken],
	);
};

/** Returns the ID from a exisitng survey from DB */
const getSurveyIdFromDb = async ({
	publicToken,
	creationToken,
}: {
	publicToken?: string;
	...
	const response = await conn.query(
		'SELECT survey_id FROM survey WHERE creation_token = ? OR public_token = ?',
		[creationToken, publicToken],
	);
...
const getTokenFromDb = async ({
	...
	const conn = await getConnection();
	const response = await conn.query(
		`SELECT creation_token, public_token FROM survey WHERE ${tokenType} = ?`,
		[token],
	);
	const converted = keysToCamelCase(response[0]);
...
const getSurveyFromDb = async (creationToken: string) => {
	const conn = await getConnection();
	const response = await conn.query(
		`SELECT survey_id, choices_type, creator_name, end_date, survey_name, survey_description
	FROM survey 
	WHERE creation_token = ?`,
		[creationToken],
	);
